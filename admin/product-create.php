...
$subCategories = DB::fetchAll("SELECT DISTINCT sub_category FROM products WHERE sub_category IS NOT NULL AND sub_category!='' ORDER BY sub_category ASC");    
...
<div class="page-header">
    ...
    <a href="product-list.php" class="btn-back">← Back to List</a>
        ...
        <form method="post" action="product-create-save.php" id="productForm">
            <div class="settings-grid">
                ...
                <section class="card">
                    ...
                    <div class="settings-grid grid-2">
                        <div class="form-group">
                            ...
                            <input type="text" name="category_new" id="categoryNew" placeholder="New category" class="hidden-input">
                        </div>
                        <div class="form-group">
                            ...
                            <select name="sub_category_select" id="subCategorySelect">
                                ...
                                <?php foreach($subCategories as $s): ?>
                                    <option value="<?= htmlspecialchars($s['sub_category']) ?>"><?= htmlspecialchars($s['sub_category']) ?></option>
                                <?php endforeach; ?>
                                <option value="__new__">+ New Sub Category</option>
                            </select>
                            <input type="text" name="sub_category_new" id="subCategoryNew" placeholder="New sub category" class="hidden-input">
                        </div>
                    </div>
                </section>
                ...
                <section class="card">
                    ...
                    <div class="form-group config-toggle" id="tierGroup">
                        <label>Tier</label>
                        <input type="number" name="tier" id="tierField" min="1">
                    </div>
                </section>
                ...
                <section class="card">
                    ...
                    <button type="button" class="btn-add" onclick="addVariant()">+ Add Variation</button>
...
function updateUI() {
    ...
    document.getElementById("variantContainer").dataset.showDuration = showDuration;
    
    document.querySelectorAll(".duration-col").forEach(el => {
        el.style.display = showDuration ? "block" : "none";
        if (!showDuration) {
            el.querySelector('input').value = "";
            el.querySelector('.duration-label').innerText = "";
        }
    });
}
...
function addVariant() {
    ...
    const showDuration = container.dataset.showDuration === "true";
    
    const row = document.createElement("div");
    row.className = "variant-row";
    row.innerHTML = `
        <div class="duration-col" style="display: ${showDuration ? 'block' : 'none'}">
            <input type="number" name="duration[]" class="duration-input" placeholder="Days" min="1">
            <small class="duration-label"></small>
        </div>
        <input type="number" name="price_idr[]" placeholder="Price IDR" required>
...
/* Event Listeners */
[categorySelect, subCategorySelect].forEach(el => {
    el.addEventListener("change", () => {
        categoryNew.style.display = categorySelect.value === "__new__" ? "block" : "none";
        subCategoryNew.style.display = subCategorySelect.value === "__new__" ? "block" : "none";
        updateUI();
    });
});

[categoryNew, subCategoryNew].forEach(el => {
